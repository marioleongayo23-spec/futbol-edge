...
from futbol_pred.ingest.api_football_players import _fair_lines_for_prop, _poisson_over


def test_poisson_over_es_monotono_y_acotado():
    p05 = _poisson_over(2.0, 0.5)
    p15 = _poisson_over(2.0, 1.5)
    p25 = _poisson_over(2.0, 2.5)
...
def test_fair_lines_generan_probabilidades_y_cuotas_reciprocas():
    ...
    fair = _fair_lines_for_prop(prop)
...
def test_media_cero_falla_cerrado_sin_cuota_over_infinita():
    fair = _fair_lines_for_prop({"r": 0, "rp": 0, "fc": 0, "fr": 0, "t": 0})
    row = fair["r"][0]
    assert row["over"] == 0.0
