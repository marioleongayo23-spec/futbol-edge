...
def test_prefinal_usa_medios_como_evidencia_sin_marcar_once_oficial(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: SimpleNamespace(text=json.dumps(response), provider="Gemini", model="test-model"),
    )
...
def test_prefinal_conserva_once_valido_si_falla_la_ia(monkeypatch):
    kickoff = datetime(2026, 8, 24, 21, tzinfo=MADRID)
    now = kickoff - timedelta(hours=3)
    match = _baseline(kickoff)
    ...
    assert match["alineacion"]["phase"] == "pre_final"
    assert match["alineacion"]["source_quality"] == "statistical_fallback"
