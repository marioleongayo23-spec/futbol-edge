...
def _row(*, feature: str, source: str, coverage: dict, available_at: str | None,
         uses: list[str], gate: str, leakage_risk: str, status: str, notes: str) -> dict:
    return {
        "feature": feature,
        "source": source,
...
def build_feature_truth_table(payload: dict) -> dict:
    ...
    features = [
        ...
        _row(
            ...
            coverage=_coverage(matches, lambda m: bool((_nested(m, "alineacion", "clave_local") or [])) and bool((_nested(m, "alineacion", "clave_visitante") or []))),
            available_at=lineup_stamp,
...
def run(path: Path = OUTPUT) -> tuple[bool, dict]:
    ...
    return ok, {"changed": True, "features": len((candidate.get("feature_truth_table") or {}).get("features") or []), "feed_valid": bool(ok), "feed_issues": report.get("issues") or []}
